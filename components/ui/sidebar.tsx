...
function SidebarProvider({
  ...
  const setWidth = React.useCallback((newWidth: number) => {
    ...
    localStorage.setItem(SIDEBAR_WIDTH_COOKIE_NAME, String(clampedWidth))
  }, [])
    ...
    >
      ...
      >
        <div
          ...
          className="bg-sidebar group-data-[variant=floating]:ring-sidebar-border group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:shadow-sm group-data-[variant=floating]:ring-1 flex size-full flex-col overflow-x-hidden"
        >
          {children}
        </div>
        <SidebarResizeHandle />
...
function SidebarResizeHandle({ className, ...props }: React.ComponentProps<"div">) {
  const { width, setWidth, state, isMobile } = useSidebar()
  ...
  // Don't show on mobile or when collapsed
  ...
  return (
    <div
      ...
      className={cn(
        "absolute top-0 bottom-0 right-0 w-1 cursor-ew-resize z-30 transition-colors duration-150",
        (isHovered || isResizing) && "bg-primary/50",
        className
      )}
      {...props}
    />
...
function SidebarInset({ className, ...props }: React.ComponentProps<"main">) {
  return (
    <main
      ...
      className={cn(
        "bg-background md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow-sm md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2 relative flex w-full flex-1 flex-col overflow-hidden min-w-0",
        className
      )}
      {...props}
    />
...
const sidebarMenuButtonVariants = cva(
  "ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground active:scale-[0.98] data-active:bg-sidebar-accent data-active:text-sidebar-accent-foreground data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground gap-2 rounded-md p-2 text-left text-sm transition-all duration-150 ease-out group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! focus-visible:ring-2 peer/menu-button flex w-full items-center overflow-hidden outline-hidden group/menu-button disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
